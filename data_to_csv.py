    ...
    for reaction in data:
        ...
        try:
            ...
            flat_reaction = {
                ...
                'from_name': reaction['from']['name'],
                'created_time': reaction['created_time'],
                'parent_id': reaction['parent']['id'] if 'parent' in reaction else None,
                'object_id': reaction['object']['id'],
            }
...
def data_to_pandas(group_name):
    ...
    for month in existing_months:
        json_posts_to_pandas(osp.join(directory, f'{month}.json'))

    directory = utils.get_dir(utils.texts_root, group_name, utils.Type.COMMENT)
    existing_months = sorted(set([utils.Month.from_str(s.replace('.json', '')) for s in os.listdir(directory)]))
    for month in existing_months:
        json_comments_to_pandas(osp.join(directory, f'{month}.json'))

    # todo: implement the rest
    directory = utils.get_dir(utils.texts_root, group_name, utils.Type.REACTION)
    existing_months = sorted(set([utils.Month.from_str(s.replace('.json', '')) for s in os.listdir(directory)]))
    for month in existing_months:
        json_reactions_to_pandas(osp.join(directory, f'{month}.json'))
